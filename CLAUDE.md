...
The user directed ongoing work onto `codex/grounded-takeoff`. Checkpoint `6e22ad2` preserves
the RSI-linked takeoff explorer before the grounded-outcomes revision. Continue on this branch;
do not commit or push to master without explicit user authorization.
...
Thirteen-tab Streamlit dashboard selected via sidebar radio (`active_tab`, `_TAB_OPTIONS`) with URL deep-linking (`?tab=<slug>`, `?to=<section>`). Each tab has its own render function, sidebar controls, and (where applicable) projection engine. Slugs (`_SLUG_FOR_TAB`): `metr`, `eci`, `ecigap`, `rli`, `rsi`, `takeoff`, `ukcyber`, `thresholds`, `employment`, `revenue`, `datacenters`, `computecap`, `pacing`.
...
subtracts finite coding dates, preserving censored draws without inf-minus-inf NaNs.
The horizon is December 31 of `tk_end_year` (default 2031); relative-duration charts have
less follow-up for late coding dates and explicitly describe that censoring.
...
ramps to the inherited milestone; overlapping endogenous coding gains use a maximum rather
than multiplying the two. Fast and successor improvements partition discoveries and each
validate a start-of-cycle snapshot. A same-compute reference freezes today's AI skills.
Full R&D gates on coding plus workflow criteria. Research acceleration has a separate chart
and 2×/5× first-crossing events; it does not require full autonomy. Sustained feedback stays
in the table but is omitted from both milestone CDF charts.

Evidence calibration uses the endpoint growth in `openai_experiment_velocity.csv`, adjusts
for assumed historical compute and coding gains, and fits a quality elasticity blended with
the prior. A supplied validated-progress ratio overrides the experiment proxy. Matched-stage
human hours can set current shares/halving times; missing inputs are zero, not invented data.
The `tk_cal_` and `tk_measure_` inputs round-trip in URLs and exports. Central resets all
Takeoff inputs and preserves RSI. Workflow fit callbacks validate before changing widgets.
...
`_rsi_experiment_draws` uses a local fixed seed so the RSI fan, threshold card, and Takeoff
reuse the same paths without perturbing global RNG. `_tk_rsi_activity` shifts these to today
and annualizes slopes. `_tk_simulate` caches by these arrays as well as model source and RSI
