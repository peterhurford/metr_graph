...
| UK Cyber | `render_ukcyber()` | `aisi_cyber_narrow.csv` → `load_ukcyber()` | success rate % (0-100 bounded, logit-projected) + open-weight lag in months |
...
| `aisi_cyber_narrow.csv` | UK AISI blog, [open-weight cyber gap post](https://www.aisi.gov.uk/blog/how-far-behind-the-frontier-are-leading-open-weight-models-on-cyber) | **Not downloadable.** AISI publishes no numbers for this chart — values were digitized from `fig1-narrow.png` by pixel analysis. To refresh, re-digitize the updated figure and re-verify against `test_digitized_dates_match_known_releases` and `test_optimistic_bracket_reproduces_aisi_published_lags` — those two are the calibration guards. Hand-editing a row is fine if AISI states a number in prose |
