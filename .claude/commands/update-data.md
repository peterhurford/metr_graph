---
description: Systematically refresh all data sources (METR, ECI, data centers, RLI, revenue) and report what changed
...
If `$ARGUMENTS` names a specific source (e.g. `eci`, `metr`, `datacenters`, `rli`, `revenue`), update only that one. Otherwise update all of them.
...
## The four external feeds + two hardcoded tables

There are four downloadable feeds and two hardcoded tables. For downloadable feeds, prefer fetching the canonical file directly over hand-transcribing.
...
5. **Verify.** After each change, sanity-check with the relevant loader:
   ...
   ```
   (use `load_frontier`, `load_data_centers`, `load_rli_data` as appropriate). Then run `pytest -q`.
